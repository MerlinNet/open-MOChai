...
func _process(delta: float) -> void:
	...
	if current_hour >= 24.0:
		...
		print("[DayNightCycle] 新的一天开始！第 %d 天" % day_count)
...
func set_time(hour: float, minute: float = 0.0) -> void:
	...
	emit_signal("time_changed", current_hour, current_minute)
	print("[DayNightCycle] 时间已设置为: %02d:%02d" % [int(current_hour), int(current_minute)])


## 设置时间流逝速度
func set_time_speed(multiplier: float) -> void:
	time_speed_multiplier = clamp(multiplier, 0.0, 100.0)
	print("[DayNightCycle] 时间速度已设置为: %.1fx" % time_speed_multiplier)


## 暂停时间流逝
func pause_time() -> void:
	auto_advance = false
	print("[DayNightCycle] 时间已暂停")


## 恢复时间流逝
func resume_time() -> void:
	auto_advance = true
	print("[DayNightCycle] 时间已恢复")
...
func register_light(light: Node) -> void:
	if light not in _registered_lights:
		...
		print("[DayNightCycle] 已注册灯光: %s" % light.name)
...
func register_all_lights_in_scene(root: Node) -> void:
	...
	print("[DayNightCycle] 共注册 %d 个灯光" % lights.size())
...
func _on_period_changed(new_period: TimePeriod, old_period: TimePeriod) -> void:
	match new_period:
		TimePeriod.DAWN:
			...
			print("[DayNightCycle] 黎明降临")
		TimePeriod.NIGHT:
			emit_signal("night_started")
			print("[DayNightCycle] 夜幕降临")
		TimePeriod.DUSK:
			emit_signal("dusk_started")
			print("[DayNightCycle] 黄昏时分")
